...
resource "aws_iam_role" "external-secrets-oidc-role" {
  ...
  managed_policy_arns = var.CREATE_EXTERNAL_SECRETS ? [
    aws_iam_policy.external-secrets-secret-manager-serviceaccount-policy[0].arn,
    aws_iam_policy.external-secrets-parameter-store-serviceaccount-policy[0].arn
  ] : []
}
...
resource "null_resource" "external-secrets-helm-chart" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      helm repo update
      echo "Installing External Secrets (v${var.ESO_VERSION})..."
      helm upgrade -i external-secrets external-secrets/external-secrets \
        ...
        --timeout 10m || { echo "Helm install failed"; exit 1; }
      echo "Waiting for Deployment and CRDs to be ready..."
      # 1. Wait for the main Deployment to be available
      kubectl --kubeconfig $KUBECONFIG_PATH -n kube-system wait --for=condition=available deploy/external-secrets --timeout=3m || { echo "Deployment wait failed"; exit 1; }
      # 2. Wait for the CRD to be established (with retry)
      for i in {1..12}; do  # Up to 1m
        if kubectl --kubeconfig $KUBECONFIG_PATH get crd clustersecretstores.external-secrets.io -o jsonpath='{.status.conditions[?(@.type=="Established")].status}' | grep -q True; then
          echo "CRD ready."
          break
        fi
        echo "CRD not ready yet... sleeping 5s (attempt $i/12)"
        sleep 5
      done
      # 3. CRITICAL WAIT: Wait for the Webhook Service Endpoints to be ready (with retry)
      echo "Waiting for external-secrets-webhook endpoints to be available..."
      for i in {1..12}; do  # Up to 1m
        if kubectl --kubeconfig $KUBECONFIG_PATH get endpoints external-secrets-webhook -n kube-system -o jsonpath='{.subsets[0].addresses[0].ip}' 2>/dev/null; then
          echo "Webhook endpoint is ready."
          break
        fi
        echo "Webhook endpoint not yet available... sleeping 5s (attempt $i/12)"
        sleep 5
      done
...
resource "null_resource" "deploy-cluster-secret-stores" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      cat <<YAML | kubectl --kubeconfig $KUBECONFIG_PATH apply -f - || { echo "Apply failed"; exit 1; }
